...
class Scanner {
   ...
   */
  void add_token(TokenType type, string literal = "") {
  ...
  void get_string() {
    ...
    if (is_at_end()) {
      CppLox::error(cur_line, "Unterminated string");
      return;
    }
  ...
  void scan_token() {
    ...
    default:
      if (isdigit(c)) {
        ...
        CppLox::error(cur_line, "Unexpected character.");
      break;
    }
  }
};
